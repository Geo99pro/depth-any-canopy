...
class PrepareDatasetFullData:
    def __init__(self, 
        ...
        if self.img.shape[-1] == 4:
            ...
            print("🟡 No NIR band found in the input image. Proceeding with RGB and CHM only.")
            self.nir = None
            ...
            # if self.args.get("weight_outliers", False):
            #     print("🟡 Weighting outliers in the CHM image.")
            #     outlier_mask = Utils.get_outlier_outlier_mask(reference_image=self.chm, threshold=1.5)

            self.chm = self.chm.astype(np.float16)  
            self.binary_mask = np.where(self.chm > 0, 1, 0).astype(np.uint8)
...
# #         break
# prepare_dataset = PrepareDatasetFullData(source_tiff_path=source_tiff_path,
#                                 reference_tiff_path=reference_tiff_path,
